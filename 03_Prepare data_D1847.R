...
clim=read.csv("~/ownCloud/Work_directory/Analysis/chapitre_2/interactions/input_clim/clim1847.csv",sep=",",dec=".", stringsAsFactors=FALSE)
data <- read.csv("~/ownCloud/Work_directory/Analysis/chapitre_2/interactions/input_chron/To1847tree.csv",sep=",",dec=".", stringsAsFactors=FALSE)
...
blue <- data[data$X>80 | data$X<20 | data$Y>80 | data$Y<20,]

tmp <- data[!is.na(data$X1991) | !is.na(data$X2000) | !is.na(data$X2010),]
ggplot(data=data, aes(X,Y))+
geom_point(aes(size=DHP11), alpha=0.5)+
geom_point(aes(size=DHP11), color="red", alpha=1, data=tmp)+
geom_point(aes(size=DHP11), color="blue", alpha=0.5, data=blue)+
