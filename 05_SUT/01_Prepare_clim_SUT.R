...
select <- c("Ptot-10", "Sftot-11", "Sftot4", "Sftot6", "DC6")
# noms qui apparaitront dans le modèle final
namesclimvar <- c("Pp10", "Sp11", "S4", "S6", "DC6")
...
clim<-read.csv("~/ownCloud/Scan & measures/Climate/PTSFSUT.csv", sep=",")
...
# Import Degree Day
clim1<-read.csv("~/ownCloud/Scan & measures/Climate/DDSUT.csv", sep=",")
...
# Import Drought Code
clim1<-read.csv("~/ownCloud/Scan & measures/Climate/DCSUT.csv", sep=",")
clim1[,"year"]<-as.integer(substr(clim1[,1],1,4))
clim1[,"month"]<-as.integer(substr(clim1[,1],6,7))
clim1<-clim1[,-1]
...
clim1<-read.csv("~/ownCloud/Scan & measures/Climate/GSLSUT.csv", sep=",")
clim1[,"month"] <- 6
clim1 <- clim1[, -c(2,3)]
...
write.csv(clim, "~/ownCloud/Work_directory/Analysis/chapitre_2/interactions/input_clim/climSUT.csv")
