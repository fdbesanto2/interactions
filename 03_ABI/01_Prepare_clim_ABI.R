...
select <- c("GSL-6", "Tmean-7", "Ptot-10", "Tmean3", "DC5", "Tmean5", "Tmean6", "Sftot6", "DC7", "Ptot7", "Tmean8", "Ptot8", "DC8")
# noms qui apparaitront dans le modèle final
namesclimvar <- c("GSLp", "Tp7", "Pp10", "T3", "DC5", "T5", "T6", "S6", "DC7", "P7", "T8", "P8", "DC8")
...
clim<-read.csv("~/ownCloud/Scan & measures/Climate/PTSFABI.csv", sep=",")
...
# Import Degree Day
clim1<-read.csv("~/ownCloud/Scan & measures/Climate/DDABI.csv", sep=",")
...
# Import Drought Code
clim1<-read.csv("~/ownCloud/Scan & measures/Climate/DCABI.csv", sep=",")
clim1[,"year"]<-as.integer(substr(clim1[,1],1,4))
clim1[,"month"]<-as.integer(substr(clim1[,1],6,7))
clim1<-clim1[,-1]
...
clim1<-read.csv("~/ownCloud/Scan & measures/Climate/GSLABI.csv", sep=",")
clim1[,"month"] <- 6
clim1 <- clim1[, -c(2,3)]
...
write.csv(clim, "~/ownCloud/Work_directory/Analysis/chapitre_2/interactions/input_clim/climABI.csv")
